...
def get_heterophase_relationship_matrix(
    ...
    """
    Computes the normalised rotation matrix for the heterophase orientation relationship described by two pairs of parallel zone axes.
    ...
    :return: The normalised rotation matrix describing the orientation relationship between bases ``A`` and ``B``.
    """
    ...
    J = dot(x * uB, inv(uA))
    s = sqrt(a[0] ** 2 + a[1] ** 2 + a[2] ** 2) / sqrt(b[0] ** 2 + b[1] ** 2 + b[2] ** 2)
    return J / s
